...
class Agent:
    ...
    def get_action(self, state):
        if np.random.uniform(0., 1.) < self.epsilon:
            ...
            # action_value_pair = [(a, self.values[(s, a)]) for s, a in self.values.keys() if s == state]

            action_value_pair = [(a, self.values[state][a]) for a in self.values[state].keys()]
    ...
    def update(self, state, action, next_state, next_action, reward):
        ...
        proba = np.exp(np.array([self.values[next_state][a] for a in self.actions]))
        proba = proba / np.sum(proba)

        value_ = 0.
        for a in self.actions:
            value_ += proba[a] * self.values[next_state][a]
